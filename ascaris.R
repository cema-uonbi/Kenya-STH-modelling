...
source('../hookworm/functions.R')

# Importing data ----------------------------------------------------------

covs <- rast("../hookworm/tif/tif_5KM.tif")
df <- fread('data/ascaris_schPrevalence.csv')
df2 <- fread("data/schoolPrevalence_coast_west.csv") 
kenya_shapefile <- st_read('../hookworm/shapefiles/gadm41_KEN_0.shp', quiet = T)
county_shapefile1 <- st_read('../hookworm/shapefiles/gadm36_ken_1.shp', quiet = T)
subcounty_shapefile1 <- st_read('../hookworm/shapefiles/gadm36_ken_2.shp', quiet = T)
actualAscaris <- fread('../../kenya_ntd/data/ascaris_summary.csv')
actualAscaris_coast_western <- fread('data/subPrevalence_coast_west.csv')
...
# elevation_rw1|ndvi_rw1|soil_moisture_rw1|soil_temp_cat	
dm2 <- dm |>
  _[, .(
    ndvi,
    prec,
...
ggvario(
  ...
  ggtitle('Empirical variogram for Ascaris') +
  theme(
    plot.title = element_text(color = 'black', hjust = .5),
    axis.title = element_text(color = 'black'))

ggsave(
  'plots/ascaris_variogram.png',
  bg = NULL,
  dpi = 1e3,
  height = 6,
  width = 10
...
spde <- inla.spde2.pcmatern(
  ...
  prior.sigma = c(5, .01)   # P(1/sigma > 5) = 0.01
)
...
stk.e1 <- inla.stack(
  ...
  effects = list(
    s = indexs,
    rr = 1:length(dm$ndvi),  # Assuming all layers have the same length
    data.frame(
      b0 = 1,
      x1 = dm2$ndvi,
      x2 = dm2$prec,
...
stk.p1 <- inla.stack(
  ...
  effects = list(
    s = indexs,
    rr = (length(pred$ndvi) + 1):(length(pred$ndvi) + nrow(pred)),
    data.frame(
      b0 = 1,
      x1 = pred$ndvi,
      x2 = pred$prec,
...
mod <- inla(
  ...
  control.inla = list(tolerance = 1e-7, strategy = "laplace", int.strategy = "grid"), 
  num.threads	= 7,
  verbose = F
)
summary(mod)
...
coeff.reg <- tidy.inla(mod, exp = F) |>
  mutate(
    terms = str_replace_all(terms, "x4", "Soil temperature ")) |> 
  filter(terms != "b0")
...
#' Rasterizing the results
ascaris1.mean <- rasterize(x = coord.p,
                        y = grid,
                        field = fitted.pred.mean)
#crs(ascaris1.mean) <- epsgKM(32737)
plot(ascaris1.mean)
#' Rasterizing the low CI
ascaris1.lower <- rasterize(x = coord.p,
                         y = grid,
                         field = fitted.pred.low)

#' Rasterizing the upper ci
ascaris1.upper <- rasterize(x = coord.p,
                         y = grid,
                         field = fitted.pred.up)
#crs(ascaris1.upper) <- epsgKM(32737)

#' The plot
rasters <- list(ascaris1.lower, ascaris1.mean, ascaris1.upper) |>
  setNames(c(
    'Lower CI',
    'Ascaris probability',
    'Upper CI'
  ))
saveRDS(rasters, "../raster results/ascaris_results.RDS")
par(mfrow = c(1,3))
map(rasters, plot)
par(mfrow = c(1,1))
...
range_std <- summary(mod)$hyperpar[4:5,][,1:5]
...
ascaris <- rast(predicted.df)
plot(raster(ascaris))

par(mfrow = c(1,3))
map(rasters, plot)
par(mfrow = c(1,1))
writeRaster(rasters$`Ascaris probability`, 'rasters/ascaris_prev.tif', overwrite = T)

# Plotting before categorizing --------------------------------------------

rast_ll <- ggplot() +
  geom_tile(data = as.data.frame(rast(ascaris1.lower) * 1e2, xy = TRUE, na.rm = TRUE),
...
rast_mean <- ggplot() +
  geom_tile(data = as.data.frame(rast(ascaris1.mean) * 1e2, xy = TRUE, na.rm = TRUE),
            aes(x = x, y = y, fill = layer)) +
  geom_sf(
    data = st_transform(county_shapefile, crs = epsgKM(32737)),
    fill = NA
...
rast_upper <- ggplot() +
  geom_tile(data = as.data.frame(rast(ascaris1.upper) * 1e2, xy = TRUE, na.rm = TRUE),
            aes(x = x, y = y, fill = layer)) +
  geom_sf(
    data = st_transform(county_shapefile, crs = epsgKM(32737)),
    fill = NA
...
raster_data <- rast(ascaris1.mean) * 1e2  
...
gg_rast <- ggplot() +
  ...
  geom_sf(data = st_transform(county_shapefile1, epsgKM(32737)), fill = NA, color = "black") +
  scale_fill_manual(values = prv_ctg_colors, name = "Prevalence (%)") +
  theme_void() +
...
subcountyLevel <- setDT(merge(subcounty_shapefile2, smat, by = 'ID', all.x = T)) |>
  ...
  st_as_sf()
saveRDS(subcountyLevel, "../raster results/ascaris_subcountyLevel.RDS")

# Plotting after population weighted --------------------------------------

actualAscaris_shp <- actualAscaris_shp |> 
...
p <- ggplot() +
  geom_sf(data = county_shapefile,
          fill = 'white',
          color = 'black') +
  geom_sf(data = st_as_sf(actualAscaris_shp),
          aes(
            fill = prv_ctg
          ),
          color = "black") +
...
p10 <- ggplot() +
  ...
  geom_sf(data = st_as_sf(subcountyLevel[!is.na(subcountyLevel$exceed10_cat),]),
          ...
          color = "black") +
  scale_fill_manual(values = prv_ctg_colors2, na.value = 'white') +
  labs(fill = "", title = "Probability of exceeding 10%") +
  theme_void() +
  theme(
    legend.text = element_text(size = 14, colour = "black"),
    plot.title = element_text(colour = "black", face = "bold", hjust = .5, size = 17)) +
...
ggsave(
  plot = p2_not,
  'plots/exceedance_not_two percent.png',
  dpi = 5e2,
  height = 12,
  width = 20,
  bg = NULL
...
res_hooks <- quantile(subcountyLevel$prev, probs = c(0.025, 0.5, 0.975), na.rm = T)
round(res_hooks, 2)
data.table(subcountyLevel) |> 
  na.omit() |> 
  _[, .N, by = prv_ctg] |> 
  _[, prev := round(N/sum(N)*1e2)][]
