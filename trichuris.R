...
df <- fread('data/trichuris_schPrevalence.csv')
...
actualTrichuris <- fread('../../kenya_ntd/data/trichuris_summary.csv')
actualTrichuris_coast_western <- fread('data/subPrevalence_coast_west.csv')
...
actualTrichuris_shp <- actualTrichuris |>
  _[ward != "", .(prev = mean(prev)), by = .(county, subcounty)] |>
  rbind(actualTrichuris |>
          _[ward == "", .(county, subcounty, prev)]) |>
  data.table() |>
  rbind(actualTrichuris_coast_western) |> 
  _[, `:=`(
    county = case_when(
...
pop_shp <- actualTrichuris_shp[, .(county, subcounty, geometry)]
...
covs2 <- brick(covs) %>%
  ...
  # crop(as(st_as_sf(bb), 'Spatial')) |> 
  # mask(as(st_as_sf(bb), 'Spatial')) 
grid <- covs2[[1]]
...
ggvario(
  ...
  ggtitle('Empirical variogram for Trichuris') +
  theme(
    plot.title = element_text(color = 'black', hjust = .5),
    axis.title = element_text(color = 'black'))

ggsave(
  'plots/trichuris_variogram.png',
  bg = NULL,
  dpi = 1e3,
  height = 6,
  width = 10
...
stk.e1 <- inla.stack(
  ...
  effects = list(
    s = indexs,
    rr = 1:length(dm$ndvi_cat),  # Assuming all layers have the same length
    data.frame(
      b0 = 1,
      x1 = dm2$ndvi_cat
    )
...
stk.p1 <- inla.stack(
  ...
  effects = list(
    s = indexs,
    rr = (length(pred$ndvi_cat) + 1):(length(pred$ndvi_cat) + nrow(pred)),
    data.frame(
      b0 = 1,
      x1 = pred$ndvi_cat
    )
...
#' Rasterizing the results
trichuris1.mean <- rasterize(x = coord.p,
                           y = grid,
                           field = fitted.pred.mean)
#crs(trichuris1.mean) <- epsgKM(32737)
plot(trichuris1.mean)
#' Rasterizing the low CI
trichuris1.lower <- rasterize(x = coord.p,
                            y = grid,
                            field = fitted.pred.low)

#' Rasterizing the upper ci
trichuris1.upper <- rasterize(x = coord.p,
                            y = grid,
                            field = fitted.pred.up)
#crs(trichuris1.upper) <- epsgKM(32737)

#' The plot
rasters <- list(trichuris1.lower, trichuris1.mean, trichuris1.upper) |>
  setNames(c(
    'Lower CI',
    'Trichuris probability',
    'Upper CI'
  ))
saveRDS(rasters, "../raster results/trichuris_results.RDS")
par(mfrow = c(1,3))
map(rasters, plot)
par(mfrow = c(1,1))
...
trichuris <- rast(predicted.df)
plot(raster(trichuris))

par(mfrow = c(1,3))
map(rasters, plot)
par(mfrow = c(1,1))
writeRaster(rasters$`Trichuris probability`, 'rasters/trichuris_prev.tif', overwrite = T)

# Plotting before categorizing --------------------------------------------

rast_ll <- ggplot() +
  geom_tile(data = as.data.frame(rast(trichuris1.lower) * 1e2, xy = TRUE, na.rm = TRUE),
...
rast_mean <- ggplot() +
  geom_tile(data = as.data.frame(rast(trichuris1.mean) * 1e2, xy = TRUE, na.rm = TRUE),
            aes(x = x, y = y, fill = layer)) +
  geom_sf(
    data = st_transform(county_shapefile, crs = epsgKM(32737)),
    fill = NA
...
rast_upper <- ggplot() +
  geom_tile(data = as.data.frame(rast(trichuris1.upper) * 1e2, xy = TRUE, na.rm = TRUE),
            aes(x = x, y = y, fill = layer)) +
  geom_sf(
    data = st_transform(county_shapefile, crs = epsgKM(32737)),
    fill = NA
...
raster_data <- rast(trichuris1.mean) * 1e2  
...
subcountyLevel <- setDT(merge(subcounty_shapefile2, smat, by = 'ID', all.x = T)) |>
  ...
  st_as_sf()
saveRDS(subcountyLevel, "../raster results/trichuris_subcountyLevel.RDS")

# Plotting after population weighted --------------------------------------

actualTrichuris_shp <- actualTrichuris_shp |> 
...
p <- ggplot() +
  geom_sf(data = county_shapefile,
          fill = 'white',
          color = 'black') +
  geom_sf(data = st_as_sf(actualTrichuris_shp),
          aes(
            fill = prv_ctg
          ),
          color = "black") +
